...
contract CaliStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        caliPerBlock = _rewardsPerBlock;
        CaliToken = ICaliToken(_token);
    }
    ...
    function pendingCali(address _user) external view returns (uint256) {
    ...
    event Claimed(address sender, address Cali, uint256 reward);
    event Deposit(address sender, address cali, uint256 amount);

    function deposit(uint256 _amount, bool _withdrawRewards)
        external
        checkStakingOver
        nonReentrant
    {
        UserInfo storage user = getUserPoolInfo[_msgSender()];
        updatePool();
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(accruedCaliShare).div(1e12).sub(
                user.rewardDebt
            );
            if (pending > 0) {
                user.pendingReward = user.pendingReward.add(pending);
                if (_withdrawRewards) {
                    safeCaliTransfer(_msgSender(), user.pendingReward);
                    emit Claimed(
                        _msgSender(),
                        address(CaliToken),
                        user.pendingReward
        ...
        if (_amount > 0) {
            IERC20(CaliTokenLP).safeTransferFrom(
                address(_msgSender()),
                address(this),
                _amount
            );
        ...
        user.rewardDebt = user.amount.mul(accruedCaliShare).div(1e12);
        emit Deposit(_msgSender(), CaliTokenLP, _amount);
    }

    event Withdraw(address sender, address cali, uint256 amount);

    function withdraw(uint256 _amount, bool _withdrawRewards)
        external
        nonReentrant
    {
        UserInfo storage user = getUserPoolInfo[_msgSender()];
        require(user.amount >= _amount, "Cali_Staking: insufficient balance");
        ...
        if (_amount > 0) {
            ...
            IERC20(CaliTokenLP).safeTransfer(_msgSender(), _amount);
        }
        user.rewardDebt = user.amount.mul(accruedCaliShare).div(1e12);
        emit Withdraw(_msgSender(), CaliTokenLP, _amount);
    }
    ...
    function claim() external nonReentrant {
        ...
        if (pending > 0 || user.pendingReward > 0) {
            ...
            safeCaliTransfer(_msgSender(), user.pendingReward);
            emit Claimed(_msgSender(), address(CaliToken), user.pendingReward);
            user.pendingReward = 0;
        }
        user.rewardDebt = user.amount.mul(accruedCaliShare).div(1e12);
    }

    function safeCaliTransfer(address _to, uint256 _amount) internal {
        IERC20(address(CaliToken)).safeTransfer(_to, _amount);
    }

    event BlockReward(address sender, uint256 blockReward);

    function setCaliPerBlock(uint256 _caliPerBlock) public onlyOwner {
        require(_caliPerBlock > 0, "block_reward_must_be_greater_than_zero");
        caliPerBlock = _caliPerBlock;
        emit BlockReward(_msgSender(), caliPerBlock);
    }
}
