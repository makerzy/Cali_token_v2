...
contract CaliBnbStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    mapping(address => UserInfo) public getUser;
    ...
    uint256 public totalReward = 5_000_000 * 10**18;
    uint256 public lastUpdateTime = 0;
    uint256 public rewardPerTokenStored = 0;

    IERC20 public CaliLPToken;
    ...
    constructor(address _caliToken, address _caliLpToken) public {
        CaliToken = ICaliToken(_caliToken);
        CaliLPToken = IERC20(_caliLpToken);
    ...
    modifier checkStakingOver() {
        require(_timestamp() < periodFinish, "Staking is over");
        _;
    }
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            UserInfo storage user = getUser[account];
            user.rewards = pendingCali(account);
    ...
    function pendingCali(address account) public view returns (uint256) {
        UserInfo storage user = getUser[account];
        if (user.lastTimeClaimed <= periodFinish)
            return
                user
                    .amount
    ...
    function claim() public nonReentrant updateReward(_msgSender()) {
        UserInfo storage user = getUser[_msgSender()];
        uint256 reward = pendingCali(_msgSender());
        if (reward > 0) {
            user.rewards = 0;
            CaliToken.mint(_msgSender(), reward);
            emit RewardPaid(_msgSender(), reward);
            totalMinted = totalMinted.add(reward);
            user.lastTimeClaimed = _timestamp();
        }
    }
        ...
        updateReward(_msgSender())
    {
        ...
        UserInfo storage user = getUser[_msgSender()];
        CaliLPToken.safeTransferFrom(_msgSender(), address(this), amount);
        ...
        updateReward(_msgSender())
    {
        ...
        CaliLPToken.safeTransfer(_msgSender(), amount);
